...
struct expression* _Owner postfix_expression(struct parser_ctx* ctx);
struct expression* _Owner cast_expression(struct parser_ctx* ctx);
struct expression* _Owner multiplicative_expression(struct parser_ctx* ctx);
struct expression* _Owner unary_expression(struct parser_ctx* ctx);
struct expression* _Owner additive_expression(struct parser_ctx* ctx);
struct expression* _Owner shift_expression(struct parser_ctx* ctx);
struct expression* _Owner relational_expression(struct parser_ctx* ctx);
struct expression* _Owner equality_expression(struct parser_ctx* ctx);
struct expression* _Owner and_expression(struct parser_ctx* ctx);
struct expression* _Owner exclusive_or_expression(struct parser_ctx* ctx);
struct expression* _Owner inclusive_or_expression(struct parser_ctx* ctx);
struct expression* _Owner logical_and_expression(struct parser_ctx* ctx);
struct expression* _Owner logical_or_expression(struct parser_ctx* ctx);
                                      ...
                                      struct argument_expression_list* p_argument_expression_list)
{
    try
    {
        ...
        if (p_current_parameter_type != NULL && !p_param_list->is_void)
        {
            ...
            {
                compiler_diagnostic_message(C_ERROR_TOO_FEW_ARGUMENTS, ctx, ctx->current, "too few arguments");
            }
            throw;
        }
    }
...
void generic_assoc_list_destroy(struct generic_assoc_list* _Obj_owner p)
{
    ...
    while (item)
    {
        struct generic_association* _Owner next = item->next;
        item->next = NULL;
...
struct generic_selection* _Owner generic_selection(struct parser_ctx* ctx)
{
    /*C23
      generic-selection:
...
struct expression* _Owner primary_expression(struct parser_ctx* ctx)
{
    /*
     primary-expression:
      identifier
    ...
    */
    ...
    try
    {
        if (ctx->current->type == TK_IDENTIFIER)
        {
            ...
            {
                compiler_diagnostic_message(C_ERROR_NOT_FOUND, ctx, ctx->current, "not found '%s'", ctx->current->lexeme);
                throw;
            }
            parser_match(ctx);
        }
        ...
        else if (ctx->current->type == TK_KEYWORD__GENERIC)
        {
            ...
            {
                compiler_diagnostic_message(C_ERROR_NO_MATCH_FOR_GENERIC, ctx, ctx->current, "no match for generic");
            }
        }
...
struct argument_expression_list argument_expression_list(struct parser_ctx* ctx)
{
    ...
    */
    ...
    struct argument_expression* _Owner p_argument_expression = NULL;
    ...
    try
    {
        ...
        while (ctx->current->type == ',')
        {
            ...
            struct argument_expression* _Owner p_argument_expression_2 = calloc(1, sizeof * p_argument_expression_2);
...
struct expression* _Owner postfix_expression_tail(struct parser_ctx* ctx, struct expression* _Owner p_expression_node)
{
    try
    {
        while (ctx->current != NULL)
        {
            if (ctx->current->type == '[')
            {
                struct expression* _Owner p_expression_node_new = calloc(1, sizeof * p_expression_node_new);
            ...
            else if (ctx->current->type == '.')
            {
                ...
                if (p_expression_node_new->left->type.type_specifier_flags & TYPE_SPECIFIER_STRUCT_OR_UNION)
                {
                    ...
                    if (p)
                    {
                        ...
                        {
                            if (p_expression_node_new->type.struct_or_union_specifier)
                            {
...
struct expression* _Owner postfix_expression_type_name(struct parser_ctx* ctx, struct type_name* _Owner p_type_name)
{
    /*
...
struct expression* _Owner unary_expression(struct parser_ctx* ctx)
{
    ...
    try
    {
        ...
        {
            struct expression* _Owner new_expression = calloc(1, sizeof * new_expression);
    ...
    try
    {
        if (first_of_type_name_ahead(ctx))
        {
            ...
            if (ctx->current->type == '{')
            {
                ...
                struct expression* _Owner new_expression = postfix_expression_type_name(ctx, p_expression_node->type_name);
            ...
            else if (is_first_of_unary_expression(ctx))
            {
                    ...
                    type_is_owner(&p_expression_node->left->type))
                {
                    if (!type_is_owner(&p_expression_node->type))
                    {
                        ...
                        {
                            //(int*) malloc(1)
                            compiler_diagnostic_message(W_OWNERSHIP_DISCARDING_OWNER, ctx, p_expression_node->first_token, "discarding _Owner pointer");
                        }
                        else
                        {
                            compiler_diagnostic_message(W_OWNERSHIP_DISCARDING_OWNER, ctx, p_expression_node->first_token, "discarding _Owner");
                        }
                    }
                }
    ...
    try
    {
                   ...
                   ctx->current->type == '%'))
        {
            ...
            if (op == '*')
            {
                ...
                {
                    compiler_diagnostic_message(C_ERROR_LEFT_IS_NOT_ARITHMETIC, ctx, ctx->current, "left * is not arithmetic");
                }
                if (!type_is_arithmetic(&new_expression->right->type))
                {
                    compiler_diagnostic_message(C_ERROR_RIGHT_IS_NOT_ARITHMETIC, ctx, ctx->current, "right * is not arithmetic");
                }
            }
            else if (op == '/')
            {
            ...
            else if (op == '%')
            {
                ...
                {
                    compiler_diagnostic_message(C_ERROR_LEFT_IS_NOT_INTEGER, ctx, ctx->current, "left is not integer");
                }
                if (!type_is_integer(&new_expression->right->type))
                {
                    compiler_diagnostic_message(C_ERROR_RIGHT_IS_NOT_INTEGER, ctx, ctx->current, "right is not integer");
                }
            ...
            {
                expression_delete(new_expression);
                compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid type multiplicative expression");
                throw;
            }
    ...
    */
    ...
    try
    {
                   ...
                   ctx->current->type == '-'))
        {
            ...
            if (new_expression == NULL)
            {
                compiler_diagnostic_message(C_ERROR_OUT_OF_MEM, ctx, ctx->current, "_Out of mem");
                throw;
            }
            ...
            {
                compiler_diagnostic_message(C_ERROR_LEFT_IS_NOT_SCALAR, ctx, operator_position, "left operator is not scalar");
            }
            if (!type_is_scalar(&new_expression->right->type))
            {
                compiler_diagnostic_message(C_ERROR_RIGHT_IS_NOT_SCALAR, ctx, operator_position, "right operator is not scalar");
            }
                ...
                */
                ...
                else
                {
                    if (left_category == TYPE_CATEGORY_POINTER || left_category == TYPE_CATEGORY_ARRAY /* || left_category == TYPE_CATEGORY_FUNCTION*/)
                    {
                        ...
                        {
                            compiler_diagnostic_message(C_ERROR_RIGHT_IS_NOT_INTEGER, ctx, ctx->current, "expected integer type on right");
                        }
                    }
                    else if (right_category == TYPE_CATEGORY_POINTER || right_category == TYPE_CATEGORY_ARRAY /*|| right_category == TYPE_CATEGORY_FUNCTION*/)
                    {
                        ...
                        {
                            compiler_diagnostic_message(C_ERROR_LEFT_IS_NOT_INTEGER, ctx, ctx->current, "expected integer type on left");
                        }
                    }
                    else
                    {
                        compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid types additive expression");
                    }
                }
            }
            else if (op == '-')
                ...
                */
                if (b_left_is_arithmetic && b_right_is_arithmetic)
                {
                    ...
                    if (code != 0)
                    {
                        ...
                        compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "internal error type_common");
                        throw;
                    }
                }
                else
                {
                    if (left_category == TYPE_CATEGORY_POINTER || left_category == TYPE_CATEGORY_ARRAY)
                    {
                        if (right_category == TYPE_CATEGORY_POINTER || right_category == TYPE_CATEGORY_ARRAY)
                        {
                            ...
                            {
                                compiler_diagnostic_message(C_ERROR_INCOMPATIBLE_POINTER_TYPES, ctx, ctx->current, "incompatible pointer types");
                            }
                        ...
                        else
                        {
                            ...
                            {
                                compiler_diagnostic_message(C_ERROR_RIGHT_IS_NOT_INTEGER, ctx, ctx->current, "right must be integer type");
                            }
                        }
                    }
                    else
                    {
                        compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid types for operator -");
                    }
                }
            }
    ...
    */
    ...
    try
    {
                   ...
                   ctx->current->type == '<<'))
        {
            ...
            {
                expression_delete(new_expression);
                compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid type shift expression");
                throw;
            }
...
struct expression* _Owner relational_expression(struct parser_ctx* ctx)
{
    ...
    */
    ...
    struct expression* _Owner p_expression_node = NULL;
    struct expression* _Owner new_expression = NULL;
    try
    {
    ...
    */
    ...
    try
    {
        ...
        while (ctx->current != NULL && ctx->current->type == '&')
        {
            ...
            if (code != 0)
            {
                compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid types and expression");
                throw;
            }
    ...
    */
    ...
    try
    {
               ...
               (ctx->current->type == '^'))
        {
            ...
            if (code != 0)
            {
                compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid types or expression");
                throw;
            }
    ...
    */
    ...
    try
    {
               ...
               (ctx->current->type == '|'))
        {
            ...
            {
                expression_delete(new_expression);
                compiler_diagnostic_message(C_ERROR_INVALID_TYPE, ctx, ctx->current, "invalid types inclusive or expression");
                throw;
            }
    ...
    */
    ...
    try
    {
               ...
               (ctx->current->type == '||'))
        {
            ...
            {
                expression_delete(new_expression);
                compiler_diagnostic_message(C_ERROR_LEFT_IS_NOT_SCALAR, ctx, ctx->current, "left type is not scalar for or expression");
                throw;
            }
            ...
            {
                expression_delete(new_expression);
                compiler_diagnostic_message(C_ERROR_RIGHT_IS_NOT_SCALAR, ctx, ctx->current, "right type is not scalar for or expression");
                throw;
            }
...
struct expression* _Owner assignment_expression(struct parser_ctx* ctx)
{
    ...
    try
    {
                   ...
                   ctx->current->type == '|='))
        {
            ...
            {
                compiler_diagnostic_message(C_ERROR_ASSIGNMENT_OF_FUNCTION, ctx, ctx->current, "assignment of function");
            }
            ...
            else
            {
                ...
                {
                    compiler_diagnostic_message(C_ERROR_ASSIGNMENT_OF_READ_ONLY_OBJECT, ctx, ctx->current, "assignment of read-only object");
                }
            }
...
void argument_expression_list_destroy(struct argument_expression_list* _Obj_owner p)
{
    ...
    while (item)
    {
        struct argument_expression* _Owner next = item->next;
        item->next = NULL;
...
{
    if (p &&
        p->expression_type == POSTFIX_FUNCTION_CALL &&
        p->left &&
        p->left->declarator &&
        p->left->declarator->name)
    {
        if (strcmp(p->left->declarator->name->lexeme, "malloc") == 0)
            return true;
    }
    return false;
}
...
bool expression_is_calloc(const struct expression* p)
{
    if (p &&
        ...
        p->left->declarator->name)
    {
        if (strcmp(p->left->declarator->name->lexeme, "calloc") == 0)
            return true;
    }
    return false;
}
...
struct expression* _Owner _Opt conditional_expression(struct parser_ctx* ctx)
{
    ...
    */
    ...
    try
    {
        ...
        {
            struct expression* _Owner p_conditional_expression = calloc(1, sizeof * p_conditional_expression);
            ...
            {
                compiler_diagnostic_message(C_ERROR_CONDITION_MUST_HAVE_SCALAR_TYPE, ctx, ctx->current, "condition must have scalar type");
            }
            ...
            else if (type_is_struct_or_union(&left_type) && type_is_struct_or_union(&right_type))
            {
                ...
                {
                    compiler_diagnostic_message(C_ERROR_INCOMPATIBLE_TYPES, ctx, p_conditional_expression->condition_expr->first_token, "incompatible types");
                }
            ...
            */
                ...
                else if (type_is_pointer(&right_type))
                {
                    ...
                    else if (!type_is_same(&left_type, &right_type, false))
                    {
                        ...
                        compiler_diagnostic_message(C_ERROR_INCOMPATIBLE_TYPES, ctx, ctx->current, "incompatible types");
                    }
                    else
                    {
                        type_swap(&p_conditional_expression->type, &right_type);
...
struct expression* _Owner _Opt constant_expression(struct parser_ctx* ctx, bool show_error_if_not_constant)
{
    ...
    {
        compiler_diagnostic_message(C_ERROR_EXPECTED_CONSTANT_EXPRESSION, ctx, ctx->current, "expected constant expression");
    }
