...
bool first_of_static_assert_declaration(struct parser_ctx* ctx)
{
    ...
    return
        ctx->current->type == TK_KEYWORD__STATIC_ASSERT ||
    ...
    */
    ...
    try
    {
        ...
        while (first_of_declaration_specifier(ctx))
        {
            ...
            if (ctx->current && ctx->current->type == TK_IDENTIFIER &&
    ...
    */
    ...
    try
    {
        ...
        else
        {
            ...
            {
                /*
                *  The objective of this visit is to initialize global objects.
    ...
    */
    ...
    try
    {
        ...
        else if (first_of_struct_or_union(ctx))
        {
            ...
            if (p_type_specifier->struct_or_union_specifier == NULL) throw;
        }
...
struct struct_or_union_specifier* owner struct_or_union_specifier(struct parser_ctx* ctx)
{
    ...
    try
    {
        ...
        if (ctx->current == NULL)
        {
            compiler_diagnostic_message(C_ERROR_UNEXPECTED, ctx, NULL, "unexpected of of file");
            throw;
        }
        ...
        if (ctx->current->type == '{')
        {
            ...
            */
            ...
            if (ctx->current->type != '}') /*not official extensions yet..missing sizeof etc*/
            {
                ...
                p_struct_or_union_specifier->member_declaration_list = member_declaration_list(ctx, p_struct_or_union_specifier, &localerror);
                if (localerror) throw;
            }
...
struct member_declaration_list member_declaration_list(struct parser_ctx* ctx, struct struct_or_union_specifier* p_struct_or_union_specifier, bool *berror)
{
    ...
    struct struct_or_union_specifier* p_struct_or_union_specifier)
{
    ...
    try
    {
        ...
        else
        {
            ...
            if (p_member_declaration->specifier_qualifier_list == NULL) throw;
...
struct enum_specifier* owner enum_specifier(struct parser_ctx* ctx)
{
    ...
    */
    ...
    try
    {
        ...
        if (parser_match_tk(ctx, TK_KEYWORD_ENUM) != 0) throw;
    ...
    long long* p_next_enumerator_value)
{
    ...
    try
    {
        ...
        if (parser_match_tk(ctx, TK_IDENTIFIER) != 0) throw;
...
struct alignment_specifier* owner alignment_specifier(struct parser_ctx* ctx)
{
    ...
    try
    {
        ...
        if (parser_match_tk(ctx, TK_KEYWORD__ALIGNAS) != 0) throw;
        if (parser_match_tk(ctx, '(') != 0) throw;
        if (first_of_type_name(ctx))
        {
...
struct atomic_type_specifier* owner atomic_type_specifier(struct parser_ctx* ctx)
{
    ...
    {
        p->token = ctx->current;
        if (parser_match_tk(ctx, TK_KEYWORD__ATOMIC) != 0) throw;
        if (parser_match_tk(ctx, '(') != 0) throw;
...
struct braced_initializer* owner braced_initializer(struct parser_ctx* ctx)
{
    ...
    */
    ...
    try
    {
        if (p_bracket_initializer_list == NULL) throw;

        p_bracket_initializer_list->first_token = ctx->current;
        if (parser_match_tk(ctx, '{') != 0) throw;
        if (ctx->current->type != '}')
        {
            p_bracket_initializer_list->initializer_list = initializer_list(ctx); //TODO error
        }
        if (parser_match_tk(ctx, '}') != 0) throw;
    }
    catch
...
struct pragma_declaration* owner pragma_declaration(struct parser_ctx* ctx)
{
    ...
    try
    {
        ...
        if (p_pragma_declaration == NULL) throw;
