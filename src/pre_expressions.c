...
static void pre_primary_expression(struct preprocessor_ctx *ctx, struct pre_expression_ctx *ectx)
{
    ...
    */
    try
    {
        ...
        {
            preprocessor_diagnostic_message(ERROR_TOKEN_NOT_VALID_IN_PREPROCESSOR_EXPRESSIONS,
                                              ctx,
                                              ctx->current,
...
static void pre_unary_expression(struct preprocessor_ctx *ctx, struct pre_expression_ctx *ectx)
{
    ...
    try
    {
                 ...
                 (ctx->current->type == '&' || ctx->current->type == '*' || ctx->current->type == '+' || ctx->current->type == '-' || ctx->current->type == '~' || ctx->current->type == '!'))
        {
            ...
            {
                preprocessor_diagnostic_message(ERROR_TOKEN_NOT_VALID_IN_PREPROCESSOR_EXPRESSIONS, ctx, ctx->current, "token '%s' is not valid in preprocessor expressions", ctx->current->lexeme);
            }
            else if (op == '&')
            {
                preprocessor_diagnostic_message(ERROR_TOKEN_NOT_VALID_IN_PREPROCESSOR_EXPRESSIONS, ctx, ctx->current, "token '%s' is not valid in preprocessor expressions", ctx->current->lexeme);
    ...
    */
    try
    {
                ...
                ctx->current->type == '-'))
        {
            ...
            if (ctx->current == NULL)
            {
                preprocessor_diagnostic_message(ERROR_UNEXPECTED_END_OF_FILE, ctx, ctx->current, "unexpected end of file");
                throw;
            }
            long long left_value = ectx->value;
            pre_multiplicative_expression(ctx, ectx);
