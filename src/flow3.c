...
/* Caps how many levels of synthetic "pointee arena" flow3 will manufacture
   ...
   for linked lists/trees) would otherwise make flow_parameter_object_init
...
enum
{
    ...
    /*
       Starting bucket count for a flow_map's hash table, and the load factor
       ...
       let chains run to hundreds of entries and made flow_map_find (called at
       every level of the parent chain by flow_map_search_up, the single
    ...
    */
    FLOW_MAP_INITIAL_BUCKETS = 8,
    FLOW_MAP_LOAD_FACTOR_PERCENT = 75,
};
...
/* What a map's debug/diagnostic name renders as (see flow_map_name_to_string).
...
{
    FLOW_MAP_ROOT,
    FLOW_MAP_TRUE_BRANCH,
    FLOW_MAP_FALSE_BRANCH,
    FLOW_MAP_BREAK_JOIN,
    FLOW_MAP_THROW_JOIN,
    FLOW_MAP_TRY_BRANCH,
    FLOW_MAP_CATCH_BRANCH,
    FLOW_MAP_SWITCH_BODY,
    FLOW_MAP_OPT_NULL,
    FLOW_MAP_OPT_NONNULL,
    FLOW_MAP_OPT_MEM_NULL,
    FLOW_MAP_OPT_MEM_NONNULL,
    FLOW_MAP_CALL_OPT_NULL,
    FLOW_MAP_CALL_OPT_NONNULL,
    FLOW_MAP_DO_WHILE_BODY_DIAG,
    FLOW_MAP_DO_WHILE_FALSE,
    FLOW_MAP_FOR_BODY_DIAG,
    FLOW_MAP_FOR_BODY_PASS1,
    FLOW_MAP_DEFAULT,
    FLOW_MAP_CASE,
    FLOW_MAP_GOTO_LABEL,
    FLOW_MAP_REL_DEAD,
    FLOW_MAP_EQ_DEAD,
    FLOW_MAP_NOT_DEAD_TRUE,
    FLOW_MAP_NOT_DEAD_FALSE,
    FLOW_MAP_OR_DEAD_TRUE,
    FLOW_MAP_OR_DEAD_FALSE,
    FLOW_MAP_AND_DEAD_TRUE,
    FLOW_MAP_AND_DEAD_FALSE,
    FLOW_MAP_MERGE_TEMP, /* short-lived local scratch map used only inside flow_map_merge_arms; never printed */
};
...
struct flow_map
{
    ...
    enum flow_map_kind kind;

    /* Only meaningful for kind == FLOW_MAP_TRUE_BRANCH/FALSE_BRANCH: the
    ...
    /* Branch identity for join correlation. A true/false map pair created for
       ...
       is already recorded by `kind` (FLOW_MAP_TRUE_BRANCH/FALSE_BRANCH), so
    ...
    /* Number of child maps ever created with this map as p_parent_map (see
       flow_map_arena_new/flow_map_arena_new_branch/flow_map_arena_new_dead).
       Used by flow_map_merge_arms's cleanup pass to tell "this map belongs
...
{
    struct flow_map* _Opt p_true;
    struct flow_map* _Opt p_false;
};

static void flow_map_name_to_string(const struct flow_map* _Opt map, struct osstream* ss);
...
static inline bool flow_value_is_same(const struct flow_alternative* a, const struct flow_alternative* b)
{
...
static bool flow_value_is_true(const struct flow_alternative* a)
{
    switch (a->value_kind)
    {
    case FLOW_VALUE_KIND_SIGNED:
...
static bool flow_value_is_false(const struct flow_alternative* a)
{
    switch (a->value_kind)
    {
    case FLOW_VALUE_KIND_SIGNED:
...
static void flow_alternatives_clear(_Clear struct flow_alternatives* vs);

static void flow_map_remove(struct flow_map* _Opt m, const struct object* obj)
{
    if (m == NULL)
...
static void flow_map_delete(struct flow_map* _Owner _Opt m);

void flow_map_arena_clear(_Clear struct flow_map_arena* a)
{
    if (a->data != NULL)
    {
        for (int i = 0; i < a->size; i++)
        {
            flow_map_delete(a->data[i]);
            a->data[i] = NULL;
        }
...
struct flow_map* _Opt flow_map_arena_new(struct flow_map_arena* a, struct flow_map* _Opt parent, enum flow_map_kind kind)
{
    try
    {
        if (a->data == NULL)
        {
            ...
            a->data = calloc(initial_capacity, sizeof(struct flow_map*));
        ...
        struct flow_map* _Owner _Opt obj = calloc(1, sizeof(struct flow_map));
        if (obj == NULL) throw;
...
static struct flow_map* _Opt flow_map_arena_new_dead(struct flow_map_arena* a, struct flow_map* parent, enum flow_map_kind kind)
{
    struct flow_map* _Opt m = flow_map_arena_new(a, parent, kind);
    if (m)
    {
...
/*
   Free-list pool for flow_alternative nodes: recycles freed slots instead
...
struct flow_alt_pool
{
    ...
    /* Block start addresses, tracked only so flow_alt_pool_free_all can
...
/*
   ...
   (sizeof(struct flow_alternative*), 8 bytes), never the alternative structs
...
static void flow_alternatives_add(struct flow_alternatives* vs, const struct flow_alternative* p_alternative)
{
    try
    {
...
static struct flow_key_alternatives* _Opt flow_map_find(const struct flow_map* m, const struct object* obj)
{
    if (!m->buckets)
    {
        return NULL;
...
static struct flow_key_alternatives* _Opt flow_map_search_up(struct flow_map* _Opt m, const struct object* obj)
{
    struct flow_map* _Opt pm = m;
    while (pm)
    {
        struct flow_key_alternatives* _Opt p = flow_map_find(pm, obj);
        if (p)
        {
            return p;
        }
...
static struct flow_key_alternatives* _Opt flow_map_find_add(struct flow_map* _Opt m, const struct object* obj)
{
    try
    {
        if (m == NULL)
        ...
        if (!m->buckets)
        {
            m->num_of_buckets = FLOW_MAP_INITIAL_BUCKETS;
        ...
        struct flow_key_alternatives* _Opt e = flow_map_find(m, obj);
        if (e)
        {
            return e;
        }
...
static void flow_map_set_object_moved(struct flow_map* _Opt m, const struct object* obj, const struct token* _Opt p_token)
{
    ...
    struct flow_key_alternatives* _Opt e = flow_map_find_add(m, obj);
    if (e == NULL) return;
...
static void flow_map_set_object_zero(struct flow_map* _Opt m, const struct object* obj, const struct token* _Opt p_token)
{
    ...
    try
    {
        ...
        struct flow_key_alternatives* _Opt p_flow_key_alternatives = flow_map_find_add(m, obj);
...
/* Mirror of flow_map_set_object_zero, but seeds each leaf member as
...
static void flow_map_apply_dtor_or_clear_effect(struct flow_map* _Opt m, const struct object* obj, bool is_clear, const struct token* _Opt p_token)
{
    ...
    {
        struct flow_key_alternatives* _Opt e = flow_map_search_up(m, obj);
        if (e)
        {
            for (int i = 0; i < e->alternatives.size; i++)
            {
...
static void flow_map_clear(_Clear struct flow_map* m)
{
    /* The bucket table and its entries go the same way here as in
       flow_map_free_entries -- that is the whole of the freeing work, so
       share it rather than keeping a second copy of the walk. */
    flow_map_free_entries(m);
    ...
    m->buckets = NULL; //lint 26  flow_map_free_entries makes null
    m->num_of_buckets = 0;
    m->num_of_entries = 0;
    m->p_parent_map = NULL;
...
/*
   ...
   but -- unlike flow_map_clear -- leaves the struct flow_map itself alive
   ...
   flow_alternative values keep pointing at `m` (for flow_map_name_to_string
   in diagnostics/static_debug, and for the ancestor-chain walk in
   flow_map_is_ancestor_or_self) for as long as the enclosing declaration is
   being analyzed. But `m`'s own bucket contents are safe to drop the moment
   nothing will look up a live value FROM `m` again: flow_alternatives_add
   ...
   *p_alternative`), never a pointer into the source entry, so a value that
   ...
   Idempotent (like flow_map_clear) -- calling it twice, or on a map with
...
static void flow_map_move_entries(struct flow_map* dest, struct flow_map* src)
{
    ...
    for (int i = 0; i < src->num_of_buckets; i++)
    {
        ...
        while (e)
        {
            ...
            struct flow_key_alternatives* _Opt d = flow_map_find_add(dest, e->p_obj_key);
            if (d)
            {
...
static bool flow_map_arm_has_entries(const struct flow_map* arm, const struct flow_map* parent)
{
...
static void flow_map_merge_arms(struct flow_map* parent, const struct flow_map* const arms[], int num_arms)
{
    ...
    struct flow_map temp = { .p_parent_map = parent, .kind = FLOW_MAP_MERGE_TEMP };
    ...
    for (int i = 0; i < num_arms; i++)
    {
        ...
        for (const struct flow_map* _Opt cur = arms[i]; cur && cur != parent; cur = cur->p_parent_map)
        {
            if (cur->buckets == NULL)
            {
                continue;
    ...
    for (int i = 0; i < objs.size; i++)
    {
        ...
        struct flow_key_alternatives* _Opt p_temp_entry = flow_map_find_add(&temp, obj);
        ...
        for (int j = 0; j < num_arms; j++)
        {
            ...
            else
            {
                /* Arm is reachable but didn't write this variable:
                   ...
                   flow_map_transfer_true_false_set) tell values that arrived
                   via this arm's path apart from an ancestor's original
                   value, without mutating the ancestor's own entry. */
                struct flow_key_alternatives* _Opt p_pre_entry = flow_map_search_up(parent, obj);
                if (p_pre_entry)
                {
    ...
    flow_map_move_entries(parent, &temp);
    flow_map_clear(&temp);
    ...
    /*
       ...
       flow_narrow_map_branch/flow_map_arena_new_branch can be shared by
       ...
       child_count is only ever incremented (see flow_map_arena_new), and
       every child a map will ever have is created before that map stops
       being `ctx->p_current_flow_map` for the branch that owns it, so by
       ...
       merge returns (every caller sets ctx->p_current_flow_map to it),
    ...
    for (int i = 0; i < num_arms; i++)
    {
        ...
        for (struct flow_map* _Opt cur = (struct flow_map*)arms[i]; cur && cur != parent; cur = cur->p_parent_map)
        {
            bool cur_is_parent_or_its_ancestor = false;
            for (const struct flow_map* _Opt panc = parent; panc; panc = panc->p_parent_map)
            {
                if (panc == cur)
                {
            ...
            flow_map_free_entries(cur);
        }
    }
}

static void flow_map_merge_a_b(struct flow_map* parent, const struct flow_map* a, const struct flow_map* b)
{
    const struct flow_map* arms[2] = { a, b };
    flow_map_merge_arms(parent, arms, 2);
}
...
static void flow_map_accumulate_into_join(struct flow_map* p_join, struct flow_map* _Opt p_src)
{
    ...
    for (const struct flow_map* _Opt cur = p_src; cur; cur = cur->p_parent_map)
    {
        if (cur->buckets == NULL)
        {
            continue;
    ...
    {
        const struct object* obj = objs.items[i];
        const struct flow_key_alternatives* _Opt p_src_entry = flow_map_search_up(p_src, obj);
        if (p_src_entry == NULL || p_src_entry->alternatives.size == 0)
        {
            continue;
        }
        struct flow_key_alternatives* _Opt p_join_entry = flow_map_find_add(p_join, obj);
...
static void flow_alternative_sprint(struct osstream* ss, const struct flow_alternative* alt)
{
...
static void flow_alternative_print(const struct flow_alternative* alt)
{
    struct osstream ss = { 0 };
...
static void flow_alternatives_print(const struct flow_alternatives* alternatives)
{
...
/* Gathers every (key, alternatives) entry live in `map`'s hash table into a
   flat array, so flow_map_debug_print can know -- before it prints any of
...
static int flow_map_collect_entries(const struct flow_map* map, const struct flow_key_alternatives** out, int max)
{
    int n = 0;
    if (map->buckets)
    {
...
/*
   ...
   flow_map_search_up answers "is there an entry for this object" by
   ...
   ("var-true") was correctly found by flow_map_search_up when reported by
   static_debug(expr) (which walks up), but flow_object_leaves_in_state's
...
*/
/* Max number of ancestor maps flow_map_debug_print will chain through
...
#define FLOW_MAP_DEBUG_MAX_CHAIN 128
...
static void flow_map_debug_print_name(const struct flow_map* map)
{
    struct osstream name_ss = { 0 };
    flow_map_name_to_string(map, &name_ss);
...
*/
static void flow_map_debug_print(const struct flow_map* _Opt map, int indent)
{
    ...
    const struct flow_map* chain[FLOW_MAP_DEBUG_MAX_CHAIN];
    int chain_len = 0;
    for (const struct flow_map* _Opt cur = map; cur; cur = cur->p_parent_map)
    {
        if (chain_len < FLOW_MAP_DEBUG_MAX_CHAIN)
            chain[chain_len] = cur;
        chain_len++;
    }
    const int printed_len = chain_len < FLOW_MAP_DEBUG_MAX_CHAIN ? chain_len : FLOW_MAP_DEBUG_MAX_CHAIN;
    ...
    for (int i = printed_len - 1; i >= 0; i--)
    {
        ...
        const struct flow_key_alternatives* entries[FLOW_MAP_DEBUG_MAX_ENTRIES] = { 0 };
        int n_entries = flow_map_collect_entries(m, entries, FLOW_MAP_DEBUG_MAX_ENTRIES);
        const int printed_entries = n_entries < FLOW_MAP_DEBUG_MAX_ENTRIES ? n_entries : FLOW_MAP_DEBUG_MAX_ENTRIES;
    ...
    if (chain_len > FLOW_MAP_DEBUG_MAX_CHAIN)
    {
        printf("%s... (chain truncated, %d more ancestors)\n", prefix, chain_len - FLOW_MAP_DEBUG_MAX_CHAIN);
    }
}
...
/*
   Write the interval [lo, hi] back into `alt`, replacing whatever relation it
   held. Returns false -- leaving `alt` untouched -- when the interval is not
   representable as a single relation (a bounded range wider than one value),
   in which case the caller keeps the alternative as it was.

   Only SIGNED alternatives are re-encoded: flow_alt_to_interval widens an
   UNSIGNED value to long long, so writing it back as SIGNED could misrepresent
   a large unsigned value.
*/
static bool flow_alt_set_interval(struct flow_alternative* alt, long long lo, long long hi)
{
...
*/
static bool flow_alt_negate(const struct flow_alternative* in,
...
static bool flow_alternative_is_zero(const struct flow_alternative* alt)
{
    return alt->value_relation == FLOW_RELATION_EQUAL && flow_value_is_false(alt);
}

static bool flow_alternative_is_false(const struct flow_alternative* alt)
{
...
/* `line` is the guard's own source line (the `if`/`while`/`&&` site doing the
   ...
   pre-narrowing alternative -- flow_alternative.line means "line of the last
...
static void flow_narrow_map_into(struct flow_map* p_dest, struct flow_map* _Opt p_before, const struct object* p_obj_key, bool true_branch, const struct token* _Opt p_token)
{
    ...
    if (p_existing == NULL || p_existing->alternatives.size == 0)
    {
        /*
           ...
           (flow_map_merge_arms) fall back to `p`'s STALE pre-loop value
        ...
        */
        struct flow_key_alternatives* _Opt p_dest_entry0 = flow_map_find_add(p_dest, p_obj_key);
    ...
    /* NOTE: this function used to also run a "correlation filter" here,
       ...
       objects continues to work correctly via flow_map_merge_arms's
    ...
    struct flow_key_alternatives* _Opt p_dest_entry = flow_map_find_add(p_dest, p_obj_key);
        ...
        if (alt->value_relation == FLOW_RELATION_EQUAL)
        {
            ...
            if (flow_value_is_true(alt) && !true_branch)
            {
                /*
                   ...
                   later lookups fall through (flow_map_search_up) to the
...
static struct flow_map* _Opt flow_map_arena_new_branch(struct flow_map_arena* a,
        struct flow_map* _Opt parent, bool is_true, const struct expression* _Opt p_expr);

/* Narrows p_obj_key's alternatives onto a new true/false child map, tagged for
   on-demand naming (see flow_map_arena_new_branch). `true_branch` doubles as
...
/*
   ...
   child maps so that flow_map_merge_arms can re-introduce the
...
static void flow_tag_branch_pair(struct flow_map* _Opt p_true, struct flow_map* _Opt p_false)
{
...
/* --- Same-predicate branch correlation -----------------------------------
   ...
   The cache (held on the visit context, flow_visit_ctx::predicate_cache) maps
...
/*
   ...
   flow_map_kind) -- for TRUE_BRANCH/FALSE_BRANCH a GCC-style
...
static void flow_map_name_to_string(const struct flow_map* _Opt map, struct osstream* ss)
{
    ...
    case FLOW_MAP_MERGE_TEMP:
        ss_fprintf(ss, "merge-temp");
        return;
    case FLOW_MAP_FOR_BODY_PASS1:
...
/* Max ancestors flow_explain_origin walks before giving up. Same
   sanity ceiling as FLOW_MAP_DEBUG_MAX_CHAIN, kept separate so the
...
static struct osstream flow_explain_origin(const struct flow_map* _Opt map)
{
    ...
    /* Collect deepest-first, then walk backwards, so the line reads in the
       order control flow actually took -- same reason flow_map_debug_print
       collects the chain into an array before printing it. */
    const struct flow_map* chain[FLOW_MAP_PATH_MAX_CHAIN];
    int chain_len = 0;
    for (const struct flow_map* _Opt cur = map; cur; cur = cur->p_parent_map)
    {
        if (chain_len < FLOW_MAP_PATH_MAX_CHAIN)
            chain[chain_len] = cur;
        chain_len++;
    }
    const int collected = chain_len < FLOW_MAP_PATH_MAX_CHAIN ? chain_len : FLOW_MAP_PATH_MAX_CHAIN;
    ...
    for (int i = collected - 1; i >= 0; i--)
    {
        ...
        if (m->kind == FLOW_MAP_ROOT || m->kind == FLOW_MAP_MERGE_TEMP)
            continue;

        struct osstream name_ss = { 0 };
        flow_map_name_to_string(m, &name_ss);
...
/*
   ...
   W_LOCATION rather than W_INFO: W_LOCATION entries attach to the preceding
   ...
   independent and survive such a suppression.

   Calling this only when the parent diagnostic() returned true is not
   enough on its own: diagnostic() returns true once the warning has been
   QUEUED, while `//lint` removal runs later, in the flow phase (see
   get_diagnostic_phase in options.c). With W_INFO the parent was removed
   and its notes were left behind -- a fully suppressed file still reported
   dozens of orphaned notes.
...
static void flow_diagnose_map_path(struct flow_visit_ctx* ctx, const struct flow_map* _Opt map)
{
    ...
    /* Collect deepest-first then emit backwards, so notes read root-first,
       ...
       flow_explain_origin / flow_map_debug_print). */
    const struct flow_map* chain[FLOW_MAP_PATH_MAX_CHAIN];
    int chain_len = 0;
    for (const struct flow_map* _Opt cur = map; cur; cur = cur->p_parent_map)
    {
    ...
    const struct expression* seen_expr[FLOW_MAP_PATH_MAX_CHAIN];
    enum flow_map_kind seen_kind[FLOW_MAP_PATH_MAX_CHAIN];
    int seen_count = 0;
...
/* Create a branch map tagged for on-demand naming: `is_true` and the
   ...
   flow_map_name_to_string() is actually asked to build the name (a
...
static struct flow_branch_pair flow_ensure_branch_pair(struct flow_visit_ctx* ctx,
        struct flow_map* _Opt p_fallback,
        struct flow_branch_pair pair,
        const struct expression* _Opt p_expr)
{
    if (pair.p_true == pair.p_false)
    {
        struct flow_map* _Opt base = pair.p_true ? pair.p_true : p_fallback;
        pair.p_true = flow_map_arena_new_branch(&ctx->flow_map_arena, base, true, p_expr);
        pair.p_false = flow_map_arena_new_branch(&ctx->flow_map_arena, base, false, p_expr);
    ...
    */
    ...
    {
        struct flow_map* _Opt fresh = flow_map_arena_new_branch(&ctx->flow_map_arena, pair.p_true, true, p_expr);
        if (fresh != NULL)
        {
            pair.p_true = fresh;
        }
    ...
    {
        struct flow_map* _Opt fresh = flow_map_arena_new_branch(&ctx->flow_map_arena, pair.p_false, false, p_expr);
        if (fresh != NULL)
        {
            pair.p_false = fresh;
        }
...
static void flow_object_init(struct flow_visit_ctx* ctx, struct object* p_object, const struct type* p_type, const struct token* _Opt p_token)
{
    ...
    {
        struct flow_key_alternatives* _Opt e = flow_map_find_add(ctx->p_current_flow_map, p_object);
        if (e == NULL) return;
...
/* force_opt: true once we're inside the pointee of an _Opt (or _Dtor-like,
   ...
   in flow_check_object_init_assigment. Rationale: an _Opt pointer's target
    ...
    try
    {
        ...
        */
                ...
                !force_opt)
        {
            ...
            struct flow_key_alternatives* _Opt ep = flow_map_find_add(ctx->p_current_flow_map, p_object);
            if (ep == NULL) throw;
                ...
                if (type_is_pointed_out(p_type))
                {
                    ...
                    if (p_pointed->members.head)
                    {
                        for (struct object* _Opt m = p_pointed->members.head; m; m = m->next)
                        {
                            ...
                            struct flow_key_alternatives* _Opt e = flow_map_find_add(ctx->p_current_flow_map, m);
                            if (e == NULL) throw;
                            flow_alternatives_clear(&e->alternatives);
                            struct flow_alternative a =
                            {
                        ...
                        //flow_map_replace_alternatives_i(ctx->p_current_flow_map,
                        //    p_pointed, NOT_APPLICABLE_VALUE, FLOW_NOT_APPLICABLE, line);
                    }
                    else
                    {

                        struct flow_key_alternatives* _Opt e = flow_map_find_add(ctx->p_current_flow_map, p_pointed);
                        if (e == NULL) throw;
                        flow_alternatives_clear(&e->alternatives);
                        struct flow_alternative a =
                        {
        ...
        *
        * Note: the condition checks `relation == FLOW_ANY` (not FLOW_UNINITIALIZED)
        * because the unconditional assignment above sets relation to FLOW_ANY.
        */
                ...
                (type_is_nullable(p_type, nullable_enabled) || force_opt))
        {
            ...
            struct flow_map* _Opt p_null_map =
                flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_OPT_NULL);
            struct flow_map* _Opt p_nonnull_map =
                flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_OPT_NONNULL);
            ...
            if (p_pointed != NULL)
            {
                ...
                *
                * We do NOT call flow_parameter_object_init here because:
                ...
                */
                ...
                struct flow_key_alternatives* _Opt eo = flow_map_find_add(ctx->p_current_flow_map, p_pointed);
                if (eo == NULL) throw;
...
/*
   ...
   (its relation/alternatives). flow_object_init only reads the parser
   ...
   them untouched. Mirrors the scalar init path (flow_check_object_init_assigment).
*/
...
static void flow_visit_if_statement(struct flow_visit_ctx* ctx, struct selection_statement* p_selection_statement)
{
    try
    {
        ...
        struct flow_map* p_before = ctx->p_current_flow_map;
        ...
        ctx->p_current_flow_map = cond_pair.p_true;
        ...
        if (p_selection_statement->else_secondary_block_opt)
        {
            ...
            if (cond_pair.p_true != NULL && cond_pair.p_false != NULL)
            {
                ...
                {
                    flow_map_merge_a_b(p_before, cond_pair.p_true, cond_pair.p_false);
                }
                else if (true_reached_the_end)
                {
                    flow_map_merge_a_b(p_before, cond_pair.p_true, cond_pair.p_true);
                }
                else if (false_reached_the_end)
                {
                    flow_map_merge_a_b(p_before, cond_pair.p_false, cond_pair.p_false);
                }
...
{
    struct flow_map* _Opt p_throw_join_map_old = ctx->p_throw_join_map;
    ...
    struct flow_map* _Opt p_throw_join = flow_map_arena_new(&ctx->flow_map_arena, p_before, FLOW_MAP_THROW_JOIN);
    ...
    /* --- visit the try body --- */
    struct flow_map* _Opt p_try_branch = flow_map_arena_new(&ctx->flow_map_arena, p_before, FLOW_MAP_TRY_BRANCH);
    if (p_try_branch == NULL)
        return; /* no map to work with */

    ctx->p_current_flow_map = p_try_branch;
    ...
    */
    ...
    if (p_try_statement->catch_secondary_block_opt)
    {
        ...
        struct flow_map* _Opt p_catch_branch = flow_map_arena_new(&ctx->flow_map_arena, p_throw_join, FLOW_MAP_CATCH_BRANCH);
        ctx->p_current_flow_map = p_catch_branch;

        flow_visit_secondary_block(ctx, p_try_statement->catch_secondary_block_opt);

        /* flow_map_arena_new is _Opt: a null p_catch_branch means the arena
    ...
    else
    {
        ...
        if (flow_map_arm_has_entries(p_throw_join, p_before))
        {
            arms[num_arms++] = p_throw_join;
        }
        if (num_arms > 0)
        {
            flow_map_merge_arms(p_before, arms, num_arms);
        }
    }
...
static void flow_visit_switch_statement(struct flow_visit_ctx* ctx, struct selection_statement* p_selection_statement)
{
    ...
    struct flow_map* _Opt old_p_initial_map = ctx->p_initial_map;
    struct flow_map* _Opt old_p_break_join_map = ctx->p_break_join_map;
    ...
    try
    {
        ...
        struct flow_map* p_before = ctx->p_current_flow_map;
        struct flow_map* _Opt p_body = flow_map_arena_new(&ctx->flow_map_arena, p_before, FLOW_MAP_SWITCH_BODY);
        struct flow_map* _Opt p_break_join = flow_map_arena_new(&ctx->flow_map_arena, p_before, FLOW_MAP_BREAK_JOIN);
...

        {
            ...
            if (reached_the_end)
            {
                arms[num_arms++] = ctx->p_current_flow_map;
            }
...
/*
   ...
   flow_map parent chain). Used to check whether a fact recorded with
   ...
   branch's own map.
*/
static bool flow_map_is_ancestor_or_self(const struct flow_map* _Opt ancestor, const struct flow_map* _Opt m)
{
    if (ancestor == NULL)
        return true; /* no filter: everything is visible */

    for (const struct flow_map* _Opt cur = m; cur; cur = cur->p_parent_map)
    {
        if (cur == ancestor)
            return true;
    }
...
/*
  ...
  self of p_origin_filter (see flow_map_is_ancestor_or_self). This
...
{
    const struct flow_key_alternatives* _Opt e = flow_map_search_up(ctx->p_current_flow_map, obj);
...
/*
   ...
   and flow_map_set_object_lifetime_ended stamps every leaf in one go. So one
...
static bool flow_object_leaves_in_state_2(struct flow_visit_ctx* ctx,
        ...
        const struct flow_map* _Opt p_origin_filter2,
        bool require_all,
        int* p_line,
        const struct flow_map* _Opt* _Opt pp_origin);
...
/*
   Same as flow_object_leaves_in_state, but accepts a SECOND, independent
   ...
   alone is correct for every shape that actually occurs.
     ...
     - dangling-pointer-to-local.c needs the POINTER ALTERNATIVE's own
       ...
       that same origin. Checking against ctx->p_current_flow_map (the
       POST-merge map) fails here: the if-branch is not ctx->p_current_flow_map's
       ancestor once merged, only a sibling of what fed into it.

     - the aliased-owner case needs ctx->p_current_flow_map (the CURRENT
   ...
   origin nor ctx->p_current_flow_map is its ancestor in those cases, so
...
static bool flow_object_leaves_in_state_2(struct flow_visit_ctx* ctx,
        ...
        bool require_all,
        int* p_line,
        /* Out: origin map of the alternative that matched, so the caller can
           render the decision path that produced the fact (see
           flow_diagnose_map_path). Set alongside *p_line, i.e. only for the
           FIRST match, matching the "see line N" the message already quotes.
           Optional -- callers that only need the line pass NULL. */
        const struct flow_map* _Opt* _Opt pp_origin)
{
    ...
    const struct flow_key_alternatives* _Opt e =
        flow_map_search_up(ctx->p_current_flow_map, p_obj);
...
/*
   ...
   flow_check_object_init_assigment loops over the source's alternatives and
   ...
   File scope rather than a member of struct flow_visit_ctx: this is private
...
static void flow_check_object_access(struct flow_visit_ctx* ctx,
                                     ...
                                     /*
                                        ...
                                        array used only up to a companion count:
                                            ...
                                            struct flow_label_state labels[100];   // flow3.h
                                            int labels_size;                        // only [0, labels_size) live

                                        `struct flow_visit_ctx ctx4 = {...}` zeroes all 100 elements, so every
                                     ...
                                     bool base_is_ptr)
{
    ...
    try
    {
        ...
        if (p_object_src->members.head)
        {
            ...
            {
                int uninit_line = 0;
                const struct flow_map* _Opt uninit_origin = NULL;
            ...
            if (members_check_ended)
            {
                ...
                if (flow_object_leaves_in_state_2(ctx, p_object_src, FLOW_LEAF_ENDED, p_origin_filter, ctx->p_current_flow_map, false, &ended_line, &ended_origin) &&
                        ended_line != 0)
                {
                    struct osstream ss = { 0 };
                    if (parent_expression_str != NULL && parent_expression_str[0] != '\0')
            ...
            {
                int moved_line = 0;
                const struct flow_map* _Opt moved_origin = NULL;
        ...
        const struct flow_key_alternatives* _Opt p_source_key_alternatives = flow_map_search_up(ctx->p_current_flow_map, p_object_src);
...
/* A pointee's members exist structurally (fabricated on first arrow-access,
   ...
   flow_seed_member_default. A member the source code never happens to read
    ...
    try
    {
                ...
                type_is_pointer(&p_object_dest->type))
        {
            struct flow_key_alternatives* _Opt e =
                flow_map_find_add(ctx->p_current_flow_map, p_object_dest);
            if (e == NULL) throw;
            flow_alternatives_clear(&e->alternatives);
            struct flow_alternative a =
            {
            ...
            */
            if (init_type == INIT_OBJ)
            {
                ...
                const struct flow_key_alternatives* _Opt e_cur =
                    flow_map_search_up(ctx->p_current_flow_map, p_object_dest);
        ...
        if (init_type == INIT_OBJ)
        {
            p_dest_key_alternatives = flow_map_find_add(ctx->p_current_flow_map, p_object_dest);
        ...
        for (int ri = 0; ri < p_src_key_alternatives->alternatives.size; ri++)
        {
                    ...
                    p_src_alternative->value.p)
            {
                ...
                if (!type_is_void_ptr(&p_object_dest->type))
                {
                    ...
                    /*
                       ...
                       with _Out (flow_dest_pointee_is_ctor, just below).
                    */
                ...
                /* Moving an owner pointer into a void* _Owner destination (e.g.
                   ...
                   members individually. flow_check_object_access above is
                   ...
                   uses (flow_check_object_at_exit) on the pointee before its
                        ...
                        p_src_alternative->value.p != NULL)
                {
                    /* The member the caller forgot to release is, by
                       ...
                       about (flow_map_search_up returns NULL and
                       flow_check_object_at_exit treats "no state" as
                    ...
                    !nullable_reported)
            {
                ...
                /* This check always tests p_object_dest's type (the
                   ...
                   so. (Compare flow_check_object_access above, which can
            ...
            if (p_src_alternative->value_relation == FLOW_RELATION_UNINITIALIZED)
            {
                ...
                /*
                   ...
                   flow_check_object_access). Storage that is uninitialized BY
...
/*
   ...
   flow_parameter_object_init invents for everything reachable through
                                       ...
                                       struct flow_discarded_owner_scan* scan)
{
    ...
    /*
       ...
       (flow_object_is_synthetic_parameter_seed) -- the fabricated
       "must be valid" placeholder flow_parameter_object_init invents
    ...
    */
    ...
    const struct flow_key_alternatives* _Opt e = flow_map_search_up(ctx->p_current_flow_map, p_object_dest);
    if (e == NULL)
    {
        return;
    }
    ...
    for (int i = 0; i < e->alternatives.size; i++)
    {
        ...
        /*
           ...
           the same seed flow_parameter_object_init gives every field
                                 ...
                                 struct expression* p_expression_src)
{
            ...
            i < p_expression_dest_key_alternatives->alternatives.size; i++)
    {
        ...
        if (p_expression_dest_alternative->value_kind == FLOW_VALUE_KIND_REF)
        {
            ...
            /* Same allocation-contract retyping the DECLARATOR-INITIALIZER path
               does (flow_visit_init_declarator). Without it, only
    ...
    if (type_is_owner(&p_expression_dest->type))
    {
        const struct flow_key_alternatives* _Opt p_src =
            flow_map_search_up(ctx->p_current_flow_map, &p_expression_src->object);
...
/* True if operand `p_expr` evaluates to exactly one constant integer,
   ...
   (the flow_evaluate_binary_arithmetic pattern -- no `data[0]`). If several
        ...
        long long* out)
{
    ...
    for (int i = 0; i < entry->alternatives.size; i++)
    {
        ...
        if (is_ref)
        {
            resolved = flow_map_search_up(ctx->p_current_flow_map, alt->value.p);
        ...
        const struct token* _Opt p_token)
{
    ...
    for (int i = 0; i < entry->alternatives.size; i++)
    {
        ...
        const struct flow_key_alternatives* _Opt obj_entry =
            flow_map_search_up(ctx->p_current_flow_map, obj);
        if (obj_entry == NULL)
            continue;
        ...
        {
            struct flow_key_alternatives* _Opt e = flow_map_find_add(p_false, obj);
            if (e != NULL)
            {
                flow_alternatives_clear(&e->alternatives);
...
static bool flow_map_is_ancestor_or_equal(const struct flow_map* anc, const struct flow_map* m)
{
    for (const struct flow_map* _Opt p = m; p != NULL; p = p->p_parent_map)
        if (p == anc)
            return true;
    return false;
}
...
static bool flow_origins_compatible(const struct flow_map* _Opt o1, const struct flow_map* _Opt o2)
{
    if (o1 == NULL || o2 == NULL || o1 == o2)
        return true;
    for (const struct flow_map* _Opt a = o1; a != NULL; a = a->p_parent_map)
    {
        if (a->branch_id == 0)
            continue;
        for (const struct flow_map* _Opt b = o2; b != NULL; b = b->p_parent_map)
        {
...
static const struct flow_map* _Opt flow_origin_more_specific(const struct flow_map* _Opt o1,
        const struct flow_map* _Opt o2)
{
    if (o1 == NULL) return o2;
    if (o2 == NULL) return o1;
    if (o1 == o2) return o1;
    if (flow_map_is_ancestor_or_equal(o1, o2)) return o2; /* o1 is ancestor -> o2 deeper */
...
/* Decide `lval == rval` (or `!=`) for one value pair. If one side is a
   ...
   flow_evaluate_equality_against_constant (which also decides NOT_EQUAL
...
/* Evaluate `a == b` (or `!=`) across ALL alternatives of both operands,
   following the flow_evaluate_binary_arithmetic iteration (per-alternative
                                        ...
                                        bool is_equal)
{
    const struct flow_key_alternatives* _Opt left_entry =
        flow_map_search_up(ctx->p_current_flow_map, &p_left->object);
    const struct flow_key_alternatives* _Opt right_entry =
        flow_map_search_up(ctx->p_current_flow_map, &p_right->object);
    ...
    */
    ...
    for (int li = 0; li < left_entry->alternatives.size; li++)
    {
        ...
        if (l_is_ref)
        {
            left_resolved = flow_map_search_up(ctx->p_current_flow_map, lalt->value.p);
        ...
        for (int lvi = 0; lvi < l_count; lvi++)
        {
            ...
            for (int ri = 0; ri < right_entry->alternatives.size; ri++)
            {
                ...
                if (r_is_ref)
                {
                    right_resolved = flow_map_search_up(ctx->p_current_flow_map, ralt->value.p);
...
/* Evaluate `a OP b` (a relational operator) across ALL alternatives of both
   operands, following the flow_evaluate_binary_arithmetic iteration: for
        ...
        char op)
{
    ...
    const struct flow_key_alternatives* _Opt left_entry = flow_map_search_up(ctx->p_current_flow_map, &p_left->object);
    const struct flow_key_alternatives* _Opt right_entry = flow_map_search_up(ctx->p_current_flow_map, &p_right->object);

    if (!left_entry || !right_entry)
    {
        struct flow_key_alternatives* _Opt e = flow_map_find_add(ctx->p_current_flow_map, &p_result->object);
        if (e == NULL) return;
                ...
                for (int rvi = 0; rvi < right_resolved->alternatives.size; rvi++)
                {
                    ...
                    */
                    ...
                    /*
                       ...
                       a plain FLOW_VALUE_KIND_SIGNED value, discarding
                       pointer identity entirely regardless of operand
                       kind. flow_alternative_can_be_zero treats that
    ...
    struct flow_key_alternatives* _Opt dst = flow_map_find_add(ctx->p_current_flow_map, &p_result->object);
    if (dst)
    {
...
static void flow_seed_member_default(struct flow_visit_ctx* ctx, struct object* _Opt member_obj, const struct token* _Opt p_token)
{
    try
    {
        if (member_obj == NULL)
            return;
        if (flow_map_search_up(ctx->p_current_flow_map, member_obj) != NULL)
        ...
        if (type_is_pointer(&member_obj->type))
        {
            ...
            if (is_nullable)
            {
                ...
                struct flow_map* _Opt p_null_map =
                    flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_OPT_MEM_NULL);
                struct flow_map* _Opt p_nonnull_map =
                    flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_OPT_MEM_NONNULL);
...
static struct flow_branch_pair flow_visit_expression(struct flow_visit_ctx* ctx, const struct expression* _Opt p_expression)
{
    ...
    if (p_expression == NULL || ctx->p_current_flow_map == NULL)
    {
    ...
    try
    {
        ...
        case EXPR_PRIMARY_DECLARATOR:
        {
                    ...
                    flow_map_search_up(ctx->p_current_flow_map, p_obj) == NULL)
            {
                ...
                {
                    struct flow_key_alternatives* _Opt e = flow_map_find_add(ctx->p_current_flow_map, p_obj);
                    if (e == NULL) throw;
                    flow_alternatives_clear(&e->alternatives);
        ...
        case EXPR_POSTFIX_DOT:
        {
            ...
            struct flow_key_alternatives* _Opt result_entry = flow_map_find_add(ctx->p_current_flow_map, &p_expression->object);
            if (result_entry == NULL) throw;
            flow_alternatives_clear(&result_entry->alternatives);

            const struct flow_key_alternatives* _Opt p_left_alternatives = flow_map_search_up(ctx->p_current_flow_map, &skip_parenthesis(p_expression->left)->object);

            struct flow_map* _Opt p_true = flow_map_arena_new_branch(&ctx->flow_map_arena, ctx->p_current_flow_map, true, p_expression);
            struct flow_map* _Opt p_false = flow_map_arena_new_branch(&ctx->flow_map_arena, ctx->p_current_flow_map, false, p_expression);
            if (p_true == NULL || p_false == NULL)
                throw;
        ...
        case EXPR_POSTFIX_ARROW:
        {
            ...
            const struct flow_key_alternatives* _Opt p_left_alternatives = flow_map_search_up(ctx->p_current_flow_map, &p_expression->left->object);
            ...
            if (p_left_alternatives != NULL && type_is_pointer(&p_expression->left->type))
            {
                        ...
                        p_left_alternatives->alternatives.data[0]->value.p != NULL)
                {
                    ...
                    const struct flow_key_alternatives* _Opt p_resolved =
                        flow_map_search_up(ctx->p_current_flow_map, p_ref);
                    if (p_resolved != NULL)
                    {
                        p_key = p_ref;
                ...
                */
                ...
                if (needs_pointee)
                {
                    ...
                    if (!type_is_void(&pointed_type))
                    {
                        ...
                        struct flow_key_alternatives* _Opt e_base = flow_map_find_add(ctx->p_current_flow_map, p_key);
                        if (e_base != NULL)
                        {
        ...
        case EXPR_POSTFIX_ARRAY:
        {
            ...
            if (object_has_known_value(&p_expression->right->object))
            {
                ...
                const struct flow_key_alternatives* _Opt p_left_alternatives =
                    flow_map_search_up(ctx->p_current_flow_map, &skip_parenthesis(p_expression->left)->object);
        ...
        case EXPR_POSTFIX_FUNCTION_CALL:
        {
            ...
            flow_visit_expression(ctx, p_expression->left);
            flow_visit_function_arguments(ctx, &p_expression->left->type, &p_expression->argument_expression_list);
            ...
            if (nullable_enabled && type_is_pointer(p_ret_type) && type_is_nullable(p_ret_type, nullable_enabled))
            {
                ...
                struct flow_map* _Opt p_null_map = flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_CALL_OPT_NULL);
                if (p_null_map == NULL)
                    throw;
                ...
                struct flow_map* _Opt p_nonnull_map = flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_CALL_OPT_NONNULL);
        ...
        case EXPR_UNARY_NOT:
        {
            ...
            */
            ...
            if (object_has_constant_value(&p_expression->right->object))
            {
                ...
                if (result != 0)
                {
                    struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena, p_before, FLOW_MAP_NOT_DEAD_FALSE);
                    if (p_dead == NULL)
                        throw;

                    return (struct flow_branch_pair)
                    {
                        ctx->p_current_flow_map, p_dead
                    };
                }
                else
                {
                    struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena, p_before, FLOW_MAP_NOT_DEAD_TRUE);
                    if (p_dead == NULL)
                        throw;

                    return (struct flow_branch_pair)
                    {
                        p_dead, ctx->p_current_flow_map
                    };
                }
            }
        ...
        case EXPR_POSTFIX_DECREMENT:
        {
            ...
            */
            ...
            for (int ri = 0; ri < p_entry->alternatives.size; ri++)
            {
                ...
                const struct flow_key_alternatives* _Opt p_resolved =
                    flow_map_search_up(ctx->p_current_flow_map, p_actual_obj);
                ...
                struct flow_key_alternatives* _Opt p_var_entry = flow_map_find_add(ctx->p_current_flow_map, p_actual_obj);
                if (p_var_entry == NULL)
                {
            ...
            flow_map_remove(ctx->p_current_flow_map, &p_operand->object);
        }
        break;
        ...
        case EXPR_UNARY_ADDRESSOF:
        {
            ...
            const struct flow_key_alternatives* _Opt p_right_alternatives = flow_map_search_up(ctx->p_current_flow_map, &p_expression->right->object);
        ...
        case EXPR_ASSIGNMENT_NOT_ASSIGN:
        {
            ...
            {
                struct flow_key_alternatives* _Opt e = flow_map_find_add(ctx->p_current_flow_map, p_left_obj);
                if (e == NULL)
                {
        ...
        case EXPR_CAST:
        {
            ...
            const struct flow_key_alternatives* _Opt p_src_entry =
                flow_map_search_up(ctx->p_current_flow_map, p_src_obj);
            ...
            for (int i = 0; all_handled && i < p_src_entry->alternatives.size; i++)
            {
                ...
                if (src_alt->value_kind == FLOW_VALUE_KIND_REF && src_alt->value.p != NULL)
                {
                    const struct flow_key_alternatives* _Opt resolved =
                        flow_map_search_up(ctx->p_current_flow_map, src_alt->value.p);
                    if (resolved == NULL)
                    {
                        all_handled = false;
                        break;
            ...
            if (all_handled && new_alts.size > 0)
            {
                struct flow_key_alternatives* _Opt dst = flow_map_find_add(ctx->p_current_flow_map, &p_expression->object);
                if (dst == NULL)
                {
                    flow_alternatives_clear(&new_alts);
                    throw;
        ...
        case EXPR_RELATIONAL_LESS_THAN:
        {
            ...
            if (fold_result != -1)
            {
                ...
                struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena,
                    ctx->p_current_flow_map,
                    FLOW_MAP_REL_DEAD);
                if (p_dead == NULL)
                    throw;
            ...
            return (struct flow_branch_pair)
            {
                ctx->p_current_flow_map, ctx->p_current_flow_map
            };
        }
        ...
        case EXPR_LOGICAL_OR:
        {
            ...
            */
                    ...
                    object_has_constant_value(&p_expression->right->object))
            {
                ...
                if (result != 0)
                {
                    struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena, p_before, FLOW_MAP_OR_DEAD_FALSE);
                    if (p_dead == NULL)
                        throw;
                ...
                {
                    struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena, p_before, FLOW_MAP_OR_DEAD_TRUE);
                    if (p_dead == NULL)
                        throw;
            ...
            ctx->p_current_flow_map = left_pair.p_false;
            ...
            */
            ...
            {
                flow_map_merge_a_b(p_or_true, left_pair.p_true, right_pair.p_true);
            }
            ...
            a 0 exactly on a path where neither disjunct holds. */
            {
                ...
                const struct flow_key_alternatives* _Opt p_right_entry =
                    flow_map_search_up(left_pair.p_false, &p_expression->right->object);
        ...
        case EXPR_LOGICAL_AND:
        {
            ...
            */
                    ...
                    object_has_constant_value(&p_expression->right->object))
            {
                ...
                if (result != 0)
                {
                    struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena, p_before, FLOW_MAP_AND_DEAD_FALSE);
                    if (p_dead == NULL)
                        throw;
                ...
                {
                    struct flow_map* _Opt p_dead = flow_map_arena_new_dead(&ctx->flow_map_arena, p_before, FLOW_MAP_AND_DEAD_TRUE);
                    if (p_dead == NULL)
                        throw;
            ...
            ctx->p_current_flow_map = left_pair.p_true;
            struct flow_branch_pair right_pair = flow_visit_expression(ctx, p_expression->right);
            ctx->p_current_flow_map = p_before;
            ...
            */
            ...
            {
                flow_map_merge_a_b(p_and_false, left_pair.p_false, right_pair.p_false);
            }
            ...
            behavior). Lets compile_assert see a 0 where either side fails. */
            {
                const struct flow_key_alternatives* _Opt p_left_entry =
                    flow_map_search_up(p_before, &p_expression->left->object);
                const struct flow_key_alternatives* _Opt p_right_entry =
                    flow_map_search_up(left_pair.p_true, &p_expression->right->object);
        ...
        case EXPR_CONDITIONAL:
        {
            ...
            cond_pair = flow_ensure_branch_pair(ctx, ctx->p_current_flow_map, cond_pair,
            ...
            */
            {
                ...
                const struct flow_key_alternatives* _Opt p_false_entry = flow_map_search_up(cond_pair.p_false, &p_false_expr->object);
    ...
    struct flow_branch_pair identity_pair = { ctx->p_current_flow_map, ctx->p_current_flow_map };
...
/*
   ...
   contracts) live in flow_visit_declaration and therefore run AFTER this
   ...
   warning fires anyway. So flow_visit_declaration calls this core and
...
static void flow_visit_do_while_statement(struct flow_visit_ctx* ctx, struct iteration_statement* p_iteration_statement)
{
    ...
    try
    {
        ...
        struct flow_map* _Opt p_pass1_exit = ctx->p_current_flow_map;

        /*
           If every path through the first pass's body unconditionally
           transferred control away, ctx->p_current_flow_map is now marked
        ...
        */
        ...
        if (body_falls_through && p_iteration_statement->expression1)
        {
            ...
            ctx->p_current_flow_map = do_cond_pair1.p_true;
        }
        ...
        else
        {
            ...
            */
            ctx->p_current_flow_map =
                flow_map_arena_new(&ctx->flow_map_arena, p_before, FLOW_MAP_DO_WHILE_BODY_DIAG);
            flow_visit_secondary_block(ctx, p_iteration_statement->secondary_block);
        }
        ...
        struct flow_map* _Opt p_false_branch_dw = do_cond_pair2.p_false ? do_cond_pair2.p_false
            : flow_map_arena_new(&ctx->flow_map_arena, p_after_body, FLOW_MAP_DO_WHILE_FALSE);
...
/*
   A loop body is unrolled a bounded number of times (one suppressed pass to
   warm the state up, one more with diagnostics on), so the exit state can
   only ever name the values reached after 0 or 1 iterations. For a variable
   the body ASSIGNS -- a counter, an accumulator -- that enumeration is not
   the whole story: the loop may run any number of times, so keeping `n == 0
   || n == 1` states a fact the source does not support.

   Left unwidened it does real harm rather than merely being imprecise: a
   later `if (n > 8)` decided statically false, so its body was reported
   unreachable and stopped being checked at all, and `n - 1` folded to a
   negative constant.

   So widen: compare the state after the FIRST pass's body with the state
   after the SECOND. A value that is the same after both is stable no matter
   how often the body runs (`a = 1` assigns 1 every time, so the exit state
   {0, 1} is already complete and must be kept). A value that changed between
   the two passes is still moving -- a counter -- and iteration three would
   name a value the enumeration does not contain, so it is set to ANY on
   every exit arm.

   Deliberately limited to numeric values. A pointer narrowed by the loop
   CONDITION (`while (p != 0)` leaving `p == 0` afterwards) is established by
        ...
        const struct token* _Opt p_token)
{
    ...
    for (struct flow_map* _Opt cur = p_pass2_exit;
    ...
    for (int i = 0; i < variants.size; i++)
    {
        ...
        for (int a = 0; a < num_arms; a++)
        {
            ...
            struct flow_key_alternatives* _Opt e = flow_map_find_add(arms[a], p_key);
            if (e == NULL)
            {
                continue;
            }
...
static void flow_visit_while_statement(struct flow_visit_ctx* ctx, struct iteration_statement* p_iteration_statement)
{
    ...
    /*
       ...
       ctx->p_current_flow_map is now marked is_unreachable by
    ...
    */
    ...
    if (body_falls_through)
    {
        ...
        w_pair2 = flow_ensure_branch_pair(ctx, ctx->p_current_flow_map, w_pair2,
                                          p_iteration_statement->expression1);
    ...
    else
    {
        ...
        */
        ...
        w_pair1_diag = flow_ensure_branch_pair(ctx, p_before, w_pair1_diag,
                                               p_iteration_statement->expression1);
        ctx->p_current_flow_map = w_pair1_diag.p_true;
        flow_visit_secondary_block(ctx, p_iteration_statement->secondary_block);
    }
...
static void flow_visit_for_statement(struct flow_visit_ctx* ctx, struct iteration_statement* p_iteration_statement)
{
    ...
    if (p_condition)
    {
        ...
        for_pair1 = flow_ensure_branch_pair(ctx, ctx->p_current_flow_map, for_pair1,
                                            p_condition);
        ctx->p_current_flow_map = for_pair1.p_true;
    }
    ...
    */
    ...
    /*
       ...
       flow_widen_loop_variant_objects' own comment gives: otherwise a body
    ...
    */
    if (body_falls_through && p_pass1_body_entry != NULL && p_pass1_exit != NULL)
    {
        /*
           ...
           1. Widen first. A value the body MOVES (`chain_len++`) must become
              ...
              later `chain_len > FLOW_MAP_DEBUG_MAX_CHAIN` would fold to false
              and report its body unreachable -- the exact harm
              flow_widen_loop_variant_objects exists to prevent. Widening must
        ...
        */
        ...
        struct flow_map* _Opt p_zero_iterations = flow_map_arena_new(&ctx->flow_map_arena,
            p_pass1_body_entry, FLOW_MAP_FOR_BODY_PASS1);
        if (p_zero_iterations != NULL)
        {
            const struct flow_map* const join_arms[2] = { p_zero_iterations, p_pass1_exit };
    ...
    if (body_falls_through && p_condition)
    {
        ...
        for_pair2 = flow_ensure_branch_pair(ctx, ctx->p_current_flow_map, for_pair2,
                                            p_condition);
        ctx->p_current_flow_map = for_pair2.p_true;
    }
    ...
    else
    {
        ...
        */
        ctx->p_current_flow_map =
            flow_map_arena_new(&ctx->flow_map_arena, p_before, FLOW_MAP_FOR_BODY_DIAG);
        flow_visit_secondary_block(ctx, p_iteration_statement->secondary_block);
    }
...
static void flow_check_file_scope_objects_at_function_exit(struct flow_visit_ctx* ctx, const struct marker* marker)
{
    ...
    for (const struct flow_map* _Opt cur = ctx->p_current_flow_map; cur != NULL; cur = cur->p_parent_map)
    {
        if (!cur->buckets)
        {
            continue;
        ...
        bool top_level)
{
    if (p_obj->members.head)
    {
        for (const struct object* _Opt member = p_obj->members.head; member; member = member->next)
        {
            flow_check_ctor_object_is_initialized_at_exit(ctx, member, param_name, marker, p_exit_token, false);
        }
        return;
    }
...
/*
   ...
   owner members instead:
     ...
     _Dtor  -- its whole contract is to release the pointee; enforced by
               flow_check_object_at_exit (called for _Dtor, immediately
               below).
   ...
   image of flow_check_ctor_object_is_initialized_at_exit: instead of
   ...
   flow_check_object_at_exit's `in_view` handling, so a _View member's own
...
/*
   ...
   against the same thing -- the synthetic pointee flow_parameter_object_init
   ...
   see flow_check_ctor_object_is_initialized_at_exit.

   _Dtor: the other half of a contract whose caller side already exists.
   flow_check_object_init_assigment ends the lifetime of x's owner members
   (FLOW_IMAGINARY_ENDED) when &x is passed to a _Dtor parameter, so the
...
static void flow_check_write_qualified_params_at_exit(struct flow_visit_ctx* ctx, const struct marker* marker, const struct token* p_exit_token)
{
            ...
            p_param = p_param->next)
    {
        ...
        const struct flow_key_alternatives* _Opt e =
            flow_map_search_up(ctx->p_current_flow_map, &p_param->declarator->object);
        ...
        for (int i = 0; i < e->alternatives.size; i++)
        {
            ...
            if (is_ctor)
            {
                flow_check_ctor_object_is_initialized_at_exit(ctx, p_alternative->value.p, param_name, marker, p_exit_token, true);
            }
...
static void flow_check_function_exit(struct flow_visit_ctx* ctx, struct jump_statement* p_jump_statement)
{
    ...
    /*
       ...
       block-scope locals. flow_defer_list_set_end_of_lifetime then calls
       flow_map_set_object_lifetime_ended on EVERY item in that list,
       ...
       becomes a sentinel) before flow_check_write_qualified_params_at_exit
       ...
       (flow_map_search_up(ctx->p_current_flow_map,
    ...
    try
    {
        if (p_jump_statement->first_token->type == TK_KEYWORD_CAKE_THROW)
        {
            ...
            */
            if (ctx->p_current_flow_map != NULL)
            {
                ctx->p_current_flow_map->is_unreachable = true;
            }
        }
        ...
        else if (p_jump_statement->first_token->type == TK_KEYWORD_GOTO)
        {
            ...
            {
                struct flow_map* _Opt p_label_map =
                    flow_map_arena_new(&ctx->flow_map_arena, ctx->p_current_flow_map, FLOW_MAP_GOTO_LABEL);
                if (p_label_map != NULL)
                {
                    /*
                       ...
                       flow_map_accumulate_into_join above. Without this,
                       the label map starts out empty and relies on its
                       parent chain (ctx->p_current_flow_map) for lookups
        ...
        else
        {
            ...
            if (p_label->constant_expression == NULL)
            {
                /* default: label — no refinement, but still must be a FRESH
                   ...
                   flow_ensure_branch_pair: if this case body ends with an
                ...
                ctx->p_current_flow_map =
                    flow_map_arena_new(&ctx->flow_map_arena, ctx->p_initial_map, FLOW_MAP_DEFAULT);
                return;
            }

            /* case N: — create a child map that records the switch variable == N */
            struct flow_map* _Opt p_case_map =
                flow_map_arena_new(&ctx->flow_map_arena,
                                   ctx->p_initial_map,
                                   FLOW_MAP_CASE);
                    ...
                    object_has_constant_value(&p_label->constant_expression->object))
            {
                ...
                {
                    struct flow_key_alternatives* _Opt e = flow_map_find_add(p_case_map, ctx->p_switch_obj_key);
                    if (e == NULL)
            ...
            ctx->p_current_flow_map = p_case_map;
        }
    }
    catch
    {
...
static void flow_visit_block_item_list(struct flow_visit_ctx* ctx, struct block_item_list* p_block_item_list)
{
    ...
    /*
       Warn once per run of dead code. ctx->p_current_flow_map is
    ...
    */
    ...
    while (p_block_item)
    {
        if (!warned_unreachable &&
                ...
                ctx->p_current_flow_map->is_unreachable)
        {
...
static void object_static_debug(struct flow_visit_ctx* ctx, const struct object* p_object, struct token* first_token, struct token* last_token)
{
    ...
    struct flow_key_alternatives* _Opt p_entry = flow_map_search_up(ctx->p_current_flow_map, p_object);
    if (p_entry == NULL)
    {
        const struct marker m =
        {
...
// Explains, in human terms, why an alternative fails flow_alternative_is_true.
...
static void flow_visit_static_assertion(struct flow_visit_ctx* ctx, struct static_assertion* p_static_assertion)
{
    if (p_static_assertion->first_token->type == TK_KEYWORD_RUNTIME_ASSERT)
    {
        /* _Assert(cond) narrows like assert: after it, only the branch
           ...
           p_before -- so it relies on flow_map_merge_arms's cleanup loop
        ...
        /* Same as flow_visit_if_statement: force pair.p_true into its own
           ...
           otherwise flow_map_merge_arms's cleanup loop, walking from an
        ...
        if (pair.p_true != NULL)
        {
            flow_map_merge_a_b(p_before, pair.p_true, pair.p_true);
        }
        ctx->p_current_flow_map = p_before;
        return;
    }
            ...
            p_static_assertion->first_token->type == TK_KEYWORD_CAKE_STATIC_DEBUG_EX)
    {
        ...
        {
            flow_map_debug_print(ctx->p_current_flow_map, 0);
        }
        else
        {
    ...
    for (int i = 0; i < e->alternatives.size; i++)
    {
                ...
                p_alternative->imaginary != FLOW_IMAGINARY_ENDED)
        {
            /* FLOW_IMAGINARY_ENDED is how a _Dtor call resolves an owner
               (flow_map_set_object_lifetime_ended, called from
            ...
            else
            {
                ...
                /* member_designator is attached to the OBJECT (e.g. the
                   ...
                   name of the declarator flow_check_object_at_exit was
...
void flow_start_visit_declaration(struct flow_visit_ctx* ctx, struct declaration* p_declaration)
{
    ...
    ctx->p_current_flow_map = flow_map_arena_new(&ctx->flow_map_arena, NULL, FLOW_MAP_ROOT);
    if (ctx->p_current_flow_map == NULL)
        return; /* no map to work with */
