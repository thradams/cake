...
/* cake emulates clang on macOS; SDK headers (e.g. TargetConditionals.h)
   require a recognized compiler. */
#define __clang__            1
#define __clang_major__      21
#define __clang_minor__      0
#define __clang_patchlevel__ 0
/* Apple compiler marker. <TargetConditionals.h> requires __GNUC__ together
   with __APPLE_CC__ (or __APPLE_CPP__) to identify the compiler; without it
   it falls through to '#error unknown compiler'. */
...
/* On Apple arm64 'long double' == 'double' (64-bit binary64). */
...
/* Expose the BSD types (u_char/u_short/u_int/u_long) on macOS. */
...
/* integer-constant builder macros (clang provides these; the SDK's
   stdint.h uses them, e.g. UINT16_MAX == __UINT16_C(65535)). */
#define __INT8_C(c)     c
