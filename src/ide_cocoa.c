...
static void view_keyDown(id self, SEL _cmd, id event)
{
    ...
    /* Ctrl+/Ctrl- and Cmd+/Cmd- resize */
    if ((mods & (UI_MOD_CTRL | UI_MOD_CMD)) && (keycode == KC_EQUAL || keycode == KC_KP_PLUS)) {
        ui_env_adjust_font_size(g_env, 1);
        return;
    }
    if ((mods & (UI_MOD_CTRL | UI_MOD_CMD)) && keycode == KC_MINUS) {
        ui_env_adjust_font_size(g_env, -1);
        return;
    }
...
int main(int argc, char** argv)
{
    ...
    ui_env_set_font_size_get_fn(g_env, get_font_size, NULL);
