...
static int __cdecl snprintf(char * _Buffer, unsigned long long _BufferCount, char * _Format, ...);
void __cdecl __va_start(char **, ...);
static int __cdecl vsnprintf(char * _Buffer, unsigned long long _BufferCount, char * _Format, char * _ArgList);
int __cdecl __stdio_common_vsprintf(unsigned long long _Options, char * _Buffer, unsigned long long _BufferCount, char * _Format, struct __crt_locale_pointers * _Locale, char * _ArgList);
static unsigned long long *__cdecl __local_stdio_printf_options(void);
static unsigned long long _OptionsStorage;
static int __cdecl vswprintf(unsigned short * _Buffer, unsigned long long _BufferCount, unsigned short * _Format, char * _ArgList);
...
void debug_wlog(unsigned short * fmt, ...)
{
    ...
    timespec_get(&ts, 1);
    rc = strftime(time_buf, 100ULL, "%D %T", gmtime(&ts.tv_sec));
    snprintf(time_buf + rc, 100ULL - rc, ".%06ld UTC", ts.tv_nsec / 1000);
    ((void)(__va_start(&args, fmt)));
    rc2 = vswprintf(buf, 1024ULL, fmt, args);
    ((void)(args = (char *)0));
    if (rc2 > 0)
    {
