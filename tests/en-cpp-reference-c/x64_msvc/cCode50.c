...
int main(void)
{
    ...
    {
        perror("fgetpos()");
        fprintf((__acrt_iob_func(2)), "fgetpos() failed in file %s at line # %d\n", "c:/users/thiago/source/repos/cake_private/tests/en-cpp-reference-c/ccode50.c", 19);
        exit(1);
    }
    ...
    if (fsetpos(fp, &pos) != 0)
    {
        ...
        {
            perror("fsetpos()");
            fprintf((__acrt_iob_func(2)), "fsetpos() failed in file %s at line # %d\n", "c:/users/thiago/source/repos/cake_private/tests/en-cpp-reference-c/ccode50.c", 30);
            exit(1);
        }
    }
    ret_code = fread(B, 8ULL, 1, fp);
