...
/*
   ...
   checked parameters (_Clear/_Dtor/_Ctor only). So `_Ctor int x;` and
...
/*
   Documented rejections (error 1940 -- qualifier on a non-pointer):
       ...
       _Clear          int a;   _Dtor int b;   _Ctor int c;   _Uninitialized int d;
   ...
   Documented rejections (error 1930 -- pointee cannot also be const):
       ...
       void c3(const _Ctor           struct S* p);
...
void ctor_it(_Ctor struct S* p);        /* callee constructs *p        */
